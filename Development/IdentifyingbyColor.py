"""
╔══════════════════════════════════════════════════════════════════════════════╗
║   SISTEMA DE DETECCIÓN DE CAÍDAS — OAK-1 + YOLOv8-pose  v17.0              ║
║   v16 + Identificación de persona objetivo por COLOR AZUL del torso         ║
╚══════════════════════════════════════════════════════════════════════════════╝
...
NUEVO EN v17 vs v16:
  ...
  ▸ CONSERVA TODO LO DE v16:
    ✓ Baseline adaptativo por mediana
    ✓ Buffer de votación de 20 frames
    ✓ Discriminador agacharse/caer (ángulo columna)
    ✓ Detección de descenso brusco
    ✓ Verificación de inmovilidad post-caída
...
ROPA RECOMENDADA:
  ✅ Buzo/sudadera AZUL REY entera (azul saturado, no marino)
  ✅ Azul eléctrico o azul cobalto
  ❌ Jeans azules (muy desaturado)
  ❌ Azul marino oscuro (se confunde con negro)
  ❌ Azul cielo claro (similar a paredes)
...
COLOR_SCORE_MIN   = 0.20   # % mínimo de píxeles azules para SER objetivo
COLOR_HISTERESIS  = 0.13   # % mínimo para MANTENERSE como objetivo (anti-oscilación)
COLOR_TIMEOUT_S   = 2.5    # segundos sin ver al objetivo → liberar y buscar otro

# Rangos HSV del azul rey (OpenCV: H: 0-179, S: 0-255, V: 0-255)
HSV_H_MIN = 100
HSV_H_MAX = 125
HSV_S_MIN = 110            # saturación mínima (filtra jeans desaturados)
HSV_S_MAX = 255
HSV_V_MIN = 50             # brillo mínimo (filtra azul marino muy oscuro)
HSV_V_MAX = 255

# Tamaño mínimo del ROI del torso (en píxeles) para considerar válido
TORSO_MIN_PIXELS = 200     # si la persona está muy lejos, el torso es muy chico
...
# ═══════════════════════════════════════════════════════════════════════════════
def _extraer_roi_torso(frame, kps, h, w):
    """
    Extrae el ROI del torso usando keypoints de hombros y caderas.
    Devuelve (roi_bgr, area_px) o (None, 0) si no es viable.
    """
    if kps.shape[0] < 13:
        return None, 0

    # Confianza mínima de los 4 keypoints del torso
    conf_min = min(kps[KP_HOMBRO_IZQ, 2], kps[KP_HOMBRO_DER, 2],
                   kps[KP_CADERA_IZQ, 2], kps[KP_CADERA_DER, 2])
    if conf_min < KP_CONF:
        return None, 0

    # Coordenadas del rectángulo del torso (en píxeles)
    x_hi = kps[KP_HOMBRO_IZQ, 0] * w
    x_hd = kps[KP_HOMBRO_DER, 0] * w
    x_ci = kps[KP_CADERA_IZQ, 0] * w
    x_cd = kps[KP_CADERA_DER, 0] * w

    y_hi = kps[KP_HOMBRO_IZQ, 1] * h
    y_hd = kps[KP_HOMBRO_DER, 1] * h
    y_ci = kps[KP_CADERA_IZQ, 1] * h
    y_cd = kps[KP_CADERA_DER, 1] * h

    x_min = int(max(0, min(x_hi, x_hd, x_ci, x_cd)))
    x_max = int(min(w, max(x_hi, x_hd, x_ci, x_cd)))
    y_min = int(max(0, min(y_hi, y_hd)))
    y_max = int(min(h, max(y_ci, y_cd)))

    # ROI demasiado pequeño → persona muy lejos o keypoints malos
    if (x_max - x_min) < 8 or (y_max - y_min) < 12:
        return None, 0
...
def servo_init():
    ...
    time.sleep(0.8)
    print(f"[SERVO Y]  ✓ {_servo_pw}µs (apuntando al suelo)")
...
def hilo_poses(pose_q, stop_event):
    ...
    while not stop_event.is_set():
        ...
        for det in dets_validas:
            ...
            kps_list.append(kps)
            roi, area = _extraer_roi_torso(frame, kps, h, w)
